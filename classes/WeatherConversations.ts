...
class WeatherConversations {
    static async defaultConversation(
        ...
        let city: string = await conversation.external(c => c.session.city);

        if(city) {
            const kb = await KeyboardBuilder.getWeatherKeyboard(true);

            await ctx0.reply('...', { reply_markup: kb });
        ...
        await ctx0.reply('Місто задано', { reply_markup: kb });
    }
    static async windConversation(
        ...
        try {
            ...
            const kb: Keyboard = await KeyboardBuilder.getWeatherKeyboard(true);

            await ctx0.reply(message, { reply_markup: kb });
        } catch(e) {
            const kb = await KeyboardBuilder.getWeatherKeyboard(false);
